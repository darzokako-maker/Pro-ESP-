...
# ============================================================
# UNICODE FİX - Güvenli kontrol ile
# ============================================================
def setup_unicode():
    """Unicode çıktısını güvenli şekilde ayarla"""
    try:
        # sys.stdout None ise düzelt
...
class PatternScanner:
    ...
    def scan_all(self, pattern, mask):
        pat_len = len(pattern)
        info(f"Pattern taramasi basliyor... (Modul boyutu: {self.size} bytes)")
...
class ProSoccerESP:
    def __init__(self):
        ...
        try:
            ...
            success("Pattern Scan ile basariyla baslatildi!")
        except Exception as e:
            warning(f"Pattern Scan basarisiz: {e}")
            info("Fallback offset'ler deneniyor...")
            try:
                self._init_with_fallback()
                success("Fallback offset'ler ile basariyla baslatildi!")
            except Exception as e2:
                error(f"Fallback da basarisiz: {e2}")
                raise RuntimeError("ESP baslatilamadi! Lutfen oyunu guncelledikten sonra tekrar deneyin.")
    ...
    def _connect_process(self):
        try:
            ...
            raise RuntimeError(f"{PROCESS_NAME} calismiyor! Lutfen oyunu baslatin.")
    ...
    def _init_with_pattern_scan(self):
        ...
        if not fname_pool:
            ...
            warning(f"FNamePool varsayilan kullaniliyor: 0x{fname_pool:X}")
    ...
    def _init_with_fallback(self):
        info("Fallback offset'ler ile baslatiliyor...")
        raise RuntimeError("Otomatik baslatma basarisiz! Lutfen diag_fname.py ile dogru offset'leri bulun.")
...
Cozum Onerileri:
1. ProSoccerOnline.exe calistigindan emin olun
2. Yonetici olarak calistirmayi deneyin
3. diag_fname.py ile dogru offset'leri bulun
    """
